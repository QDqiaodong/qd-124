...
后端测试（JUnit 5 + Mockito）覆盖热门型号 Redis 缓存在支架新增、改名、删除后的失效逻辑，以及 Redis 异常不影响支架保存的降级路径；另含换线改挂端到端测试（预检按目标机规则分项、确认后通过项一次改挂且冲突项留在源设备、全程无未绑定中间态），以及换模批次登记与放行联锁端到端测试（未写批次/型号不在允许清单时批量挂接与改挂整单拦截、换新批次后旧记录不再放行、按设备翻历史）。前端测试（Vitest + Vue Test Utils）回归支架档案操作后型号建议刷新、绑定/规则页型号建议实时性、改挂预检请求、确认后设备占用刷新，以及换模批次登记/历史弹窗与批量绑定批次闸门前端拦截。
