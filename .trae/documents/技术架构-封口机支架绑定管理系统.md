...
| PUT | /api/equipment/{id}/rule | 保存设备配套规则（最大数量、允许支架型号、允许模具型号、长宽范围） |
| POST | /api/equipment/{id}/rule/diagnose | 规则变更影响诊断（不落库）：预览受影响的已绑定支架，区分需人工处理的存量绑定与仅影响后续绑定的条目 |
| POST | /api/equipment/{id}/mold-batches | 换模后登记当前模具批次（型号须在允许模具型号清单内），新增历史记录 |
| GET | /api/equipment/{id}/mold-batches | 按设备分页翻历史换模记录（换模时间倒序，第一条为当前批次） |
...
erDiagram
    ...
    EQUIPMENT ||--o{ MOLD_BATCH_RECORD : "换模历史"
    EQUIPMENT {
        bigint id PK
        varchar equipment_code UK "设备编号"
        varchar equipment_name "设备名称"
        varchar allowed_mold_models "允许模具型号清单"
    ...
    MOLD_BATCH_RECORD {
        bigint id PK
        bigint equipment_id FK "封口设备ID"
        varchar batch_no "模具批次号"
        varchar mold_model "模具型号"
        datetime change_time "换模时间"
...
> 当前模具批次 = 该设备 `change_time` 最新（`id` 倒序兜底）的一条换模记录。
> 批量挂接（`/api/binding/batch-check`、`/api/binding/batch-confirm`）与换线改挂
> （`/api/binding/rehang-check`、`/api/binding/rehang-confirm`）在型号/尺寸/容量校验之前先过批次闸门
> `MoldBatchService.evaluateGate`：无当前批次或其型号不在 `allowed_mold_models` 内时整单判为冲突，
> 预检结果通过 `moldBatchGate` 字段返回；确认接口以同一闸门重新强制校验。
